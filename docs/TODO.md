...
- [ ] **Fase 2 — Apply con red de seguridad (attach/detach de la TV)**: traer `apply.rs` **y** la mitad de `topology.rs` que hoy no está (cache + merges + persistencia = la maquinaria del re-attach). **Re-implementar las DOS piezas de seguridad**: resume-listener (`invalidate_backend_cache()` al despertar) **y** watchdog de auto-rollback — el manager del crate puro es **pasivo**, guarda el deadline pero NO dispara. Front: countdown Confirmar/Revertir. **Leer `docs/DECISIONS.md` → "Doctrina CCD heredada" ANTES de escribir una línea.** Al instanciar `MonarchDisplayManager`, apuntar el store al APPDATA de **Millennium** (el default escribe sobre el config real de Monarch). Chat nuevo.
- [ ] **Fase 3 — Perfiles, ajustes y watcher en vivo**: `ConfigStore` sobre `JsonStore`, tabs de perfiles, watcher por evento (`WM_DISPLAYCHANGE`, necesita la ventana oculta de `app/events.rs` de Monarch), lienzo de arrastre (net-new).
