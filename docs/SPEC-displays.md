...
- Estado: EN CURSO — **Fase 0 ✅** (CI verde, `5585175`) · **Fase 1 ✅ IMPLEMENTADA Y VERIFICADA EN HARDWARE** (`76afb8a`, run 29754851028) · **Fase 2 ✅ IMPLEMENTADA Y VERIFICADA EN HARDWARE** (2026-07-20, commit `9534822`, 3 workflows verdes; en el desktop de 3 displays: la TV se apaga y se prende, **si no se confirma vuelve sola**, si se confirma se queda, y los otros dos monitores no se movieron) · **Fase 3 ✅ IMPLEMENTADA y VERIFICADA en hardware (núcleo)** (2026-07-21, **v1.2.0**; perfiles + ajustes + watcher `WM_DISPLAYCHANGE` + lienzo de arrastre opción A). En el desktop de 3 displays, con el `.exe` instalado por el auto-updater: guardar/cargar perfil, acomodar en el lienzo y aplicar, y el **auto-revert vuelve solo** — OK. Pendientes menores (ver `docs/TODO.md`): plazo desde AJUSTES, refresco en vivo al enchufar/desenchufar, y regresión (transferencia/CPU). Con esos sub-checks, el SPEC queda COMPLETO y se archiva. ⚠️ Desvíos del plan, todos justificados en `docs/DECISIONS.md`: vendor por copia y no `git subtree` (ADR-001), `topology.rs`/`apply.rs` recién en la Fase 2 (ADR-002), sin campo en `AppState` (ADR-005), **sin la persistencia binaria del snapshot** (ADR-008), el watchdog reescrito en vez de copiado (ADR-009), `displays_apply_layout` en la Fase 3 con el lienzo (ADR-012), y el watcher de dos canales que refresca sin invalidar (ADR-013).
- Fecha: 2026-07-18
