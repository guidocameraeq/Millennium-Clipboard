...
- Estado: EN CURSO — **Fase 0 ✅** (CI verde, `5585175`) · **Fase 1 ✅ IMPLEMENTADA Y VERIFICADA EN HARDWARE** (`76afb8a`, run 29754851028) · **Fase 2 ✅ IMPLEMENTADA Y VERIFICADA EN HARDWARE** (2026-07-20, commit `9534822`, 3 workflows verdes; en el desktop de 3 displays: la TV se apaga y se prende, **si no se confirma vuelve sola**, si se confirma se queda, y los otros dos monitores no se movieron) · **Fase 3 pendiente**. ⚠️ Desvíos del plan, todos justificados en `docs/DECISIONS.md`: vendor por copia y no `git subtree` (ADR-001), `topology.rs`/`apply.rs` recién en la Fase 2 (ADR-002), sin campo en `AppState` (ADR-005), **sin la persistencia binaria del snapshot** (ADR-008), el watchdog reescrito en vez de copiado (ADR-009) y `displays_apply_layout` corrido a la Fase 3 (no hay UI que arme un layout hasta el lienzo; meterlo ahora era código muerto).
- Fecha: 2026-07-18
